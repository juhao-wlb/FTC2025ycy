...
public class PIDMotionProfileController extends PIDFController {
    ...
    public PIDMotionProfileController(double kp, double ki, double kd, double kS, double kV, double kA, double kG) {
        super(kp, ki, kd, 0);
        this.kG = kG;
        this.kS = kS;
        this.kV = kV;
    ...
    public double calculate(double pv){
        ...
        double feedforward = Math.copySign(kS, PIDPower) + kV * set_velocity + kA * set_accleration + (gravityMode==GravityMode.CONSTANT? kG : kG * Math.cos(pv / TicksPerRev * Math.PI*2));
        return PIDPower + feedforward;
    }
    public PIDMotionProfileController copy(){
        PIDMotionProfileController controller = new PIDMotionProfileController(getP(), getI(), getD(), kS, kV, kA, kG);
