...
//    public static boolean useFeedforward = true;
    public static PIDMotionProfileController.GravityMode gravityMode = PIDMotionProfileController.GravityMode.CONSTANT;
...
//    void
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (!read_only) {
                double motorPowerFront = motorPID.calculate(nowPositionTicks, set_position);//motorPID.calculate(nowPositionTicks,set_position);
//                double Feedforward = 0;
//                if(useFeedforward){
//                    Feedforward = Math.copySign(kS, nowVelocityRadPS) + kV * set_velocity + kA * set_accleration + (gravityMode== PIDMotionProfileController.GravityMode.CONSTANT? kG : kG * Math.cos(nowPositionTicks / 12 + horizontalRotationRads));
//                }
